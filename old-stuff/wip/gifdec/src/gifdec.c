...
read_image_data(gd_GIF *gif, int interlace)
{
    ...
    while (frm_off < frm_size) {
        ...
        for (i = 0; i < str_len; i++) {
            ...
            // if (((gif->fy + y) * gif->width + gif->fx + x) > (gif->width * gif->height * 4)) {
            //     fprintf(stderr, "[OOB] write\n");
            //     table->entries[100000] = (Entry) {4, 4, 4};
            //     assert(0);
            // }
            if (entry.prefix == 0xFFF)
                break;
            else {
                // if (entry.prefix >= table->nentries || key == table->nentries) {
                //     fprintf(stderr, "[OOB] read\n");
                //     table->entries[100000] = (Entry) {4, 4, 4};
                //     assert(0);
                // }
