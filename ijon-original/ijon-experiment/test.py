...
def build_binaries():
    ...
    for name in binaries:
        subprocess.run(['bash', '-c', f'/home/dev/afl-clang-fast ./src/{name}.c -o ./binaries/{name} > /dev/null 2>&1'])


async def run_fuzzers():
    for i in range(len(in_dirs)):
        cmd = f"bash -c 'export AFL_BENCH_UNTIL_CRASH=1 AFL_SKIP_CPUFREQ=1 AFL_I_DONT_CARE_ABOUT_MISSING_CRASHES=1; tmux new-session -d -s {binaries[i]} /home/dev/afl-fuzz -i {in_dirs[i]} -o {out_dirs[i]} -- ./binaries/{binaries[i]}  > /dev/null 2>&1'"
